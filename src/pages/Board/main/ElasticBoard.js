...
function ElasticBoard() {
  ...
  function paging(pagevalue, currentPage, pageSize) {
    ...
    if (first > 10) {
      buttons.push(
        <button className="pagination-button" value="1" key="1">
          Top
        </button>
      );
      buttons.push(
        // <Link to={`/board?b_type=${selectTabs}&viewCnt=${pageSize}&search=${currentPage}`}>
        <button className="pagination-button" value={prev} key={prev}>
          Prev
        </button>
        //</Link>
      );
    ...
    for (let j = first; j <= last; j++) {
      if (currentPage === j) {
        buttons.push(
          <button className="pagination-button" value={j} key={j}>
            {j}
          </button>
        );
      } else if (j > 0) {
    ...
    if (next > 10 && next < lowpage) {
      buttons.push(
        <button className="pagination-button" value={next} key={next}>
          Next
        </button>
      );

      buttons.push(
        <button className="pagination-button" value={lowpage} key={lowpage}>
          End
        </button>
      );
    }
                ...
                <div>
                  <ul className="tabs">
                    {tabEvent.map((el, index) => (
                      <Link
                        key={el.id}
                        to={`/board?b_type=${el.link}&viewCnt=${pageCount}&search=${currentPage}`}
                      >
                        <li
                          className={
                            index === tabsCss ? "tab-link current" : "tab-link"
                          }
                          id={el.id}
                  ...
                  {elasticResults.map((hit) => {
                    ...
                    return (
                      <tr key={hit._id}>
                        ...
                        <td
                          style={{ textAlign: "center", padding: "0 0 0 10px" }}
                        >
                          <a href={"/detail?board_id=" + hit._id}>
                            {hit._source.b_title}
                          </a>
                        </td>
                        <td>{member_id}</td>
