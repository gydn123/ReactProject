...
const TabMenu = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8080/amusement/rideDetail/${rides_id}`)
      .then((response) => setRideDetail(response.data));
    axios
      .get(`http://localhost:8080/amusement/ridesList/${amuse_id}`)
      .then((response) => setRidesList(response.data));
    axios
      .get(`http://localhost:8080/amusement/inAvgGradeList`)
    ...
    },
    {
      ...
      content: (
        ...
        >
          ...
          <li
            style={{ textAlign: "left", padding: "10px", fontSize: "1.3rem" }}
          >
            <span style={{ fontWeight: "bold" }}>위치:</span>&nbsp;&nbsp;
            {rideDetail?.r_location}
          </li>
          <li
            style={{ textAlign: "left", padding: "10px", fontSize: "1.3rem" }}
          >
          ...
          <Slider {...settings}>
            {inAvgGradeList.map((amuse) => (
              <Card
                ...
                path="amusement"
                name={amuse.a_name}
                country={amuse.a_country}
                img={amuse.a_img}
                avg={amuse.avg_grade}
                review_cnt={amuse.review_cnt}
              />
            ))}
          </Slider>
        </Container>
    ...
    <Container>
      <div className="tab-menu">
        {menuArr?.map((tab, index) => (
          <li
            key={index}
            className={index === selectedTab ? "sub-menu focused" : "sub-menu"}
