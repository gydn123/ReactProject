...
<script type="text/javascript">
	...
	$(function() {
		...
		var order_id = confirmButton.value;
		var totalPoint = document.querySelector('.Point').value;
		var m_point = parseFloat(totalPoint.replace(/[^0-9]/g, ''));
		var use_point = document.getElementById('use_point').value;

		confirmButton.addEventListener('click', function() {
			var confirmResult = confirm("구매 확정 하시겠습니까?");
			if (confirmResult) {
				if (m_point != 0) {
					$.ajax({
						type : 'POST',
			...
			<article class="offset-1 col-8">
				...
				<c:forEach items="${data}" var="d" varStatus="loop">
					<c:if test="${d['quantity'] != 0}">
						...
						<c:set var="totalPoint" value="${totalPoint + cardPoint}" />
					</c:if>
				</c:forEach>
				<div class="row" id="table-row">
					<c:forEach items="${data}" var="d" varStatus="loop">
						<c:if
							test="${d['quantity'] != 0 or data[loop.index + 1]['quantity'] != 0 or data[loop.index + 2]['quantity'] != 0}">
							<c:if
								...
								<h4 class="mb-3">
									주문번호: ${d['order_id']}
									<c:choose>
										<c:when test="${d['checkorder'] != true}">
											<button class="btn btn-outline-success" type="button"
												id="confirm" value="${data[0].order_id}">구매 확정</button>
										</c:when>
								...
								<div class="card">
									<div class="card-body">
										...
										<p class="card-text">
											결제금액 :
											<c:choose>
											<c:when test="${d['use_point'] == null or d['use_point'] == 0.00 }">
											<fmt:formatNumber
												value="${(d['ticket_price']*(1-d['discount']))*d['quantity']}"
												pattern="#,##0₩" type="currency" currencySymbol="₩" />
											</c:when>
											<c:when test="${d['use_point'] != 0.00 }">
											<fmt:formatNumber
												value="${(d['ticket_price']*(1-d['discount']))*d['quantity']}"
												pattern="#,##0₩" type="currency" currencySymbol="₩" />-
											<fmt:formatNumber value="${d['use_point']}" pattern="#,##0"/>포인트 =
											<fmt:formatNumber
										...
										<c:set var="Point"
											value="${(d['ticket_price']*(1-d['discount']))*d['quantity']*0.05}" />
										<p
											data-point="<fmt:formatNumber value="${Point}" pattern="#,##0" />">
											구매 확정 시 적립 포인트 :
											<fmt:formatNumber value="${Point}" pattern="#,##0" />
										</p>
									</div>
								</div>
							</div>
						</c:if>
						<input type="hidden" value="${d['use_point']}" id="use_point">
					</c:forEach>
				</div>
				<input type="hidden"
					value="<fmt:formatNumber value="${totalPoint}" pattern="#,##0" />"
					class="Point">
			</article>
		</div>
	</main>
